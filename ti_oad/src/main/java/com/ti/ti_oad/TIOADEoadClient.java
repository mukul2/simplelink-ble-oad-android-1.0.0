...
public class TIOADEoadClient {
  ...
  public void sendHeaderToDevice() {
    ...
    System.out.println(fileReader.getHeaderForImageNotify());
  ...
  public void sendEoadDataBlock(long block) {
    for(int i = 0 ; i <  100 ; i ++){
    //  Log.i("pos",""+i+" "+ fileReader.getRawImageData()[i]);
    }



        int dataSize = TIOADEoadBlockSize - 4;
       // Log.i("file3", "dataSize "+dataSize);
        int readSize = dataSize;
        //Log.i("file3", "readSize "+readSize);
        ...
        for (int ii = 0; ii < 4; ii++) {
         // Log.i("setting", String.valueOf( TIOADEoadDefinitions.GET_BYTE_FROM_UINT32(block, ii)));
          //Log.i("setting", "at "+ ii);
         // Log.i("settingsingle1", String.valueOf(TIOADEoadDefinitions.GET_BYTE_FROM_UINT32(block, ii)));
          buffer[ii] = TIOADEoadDefinitions.GET_BYTE_FROM_UINT32(block, ii);
        }
       // Log.i("settingsingle",  Arrays.toString(buffer));
      //  Log.i("fileblock", ""+block+" "+Arrays.toString(buffer));
   ...
   // Log.d("filewrite",String.valueOf(dataSize)+"x"+String.valueOf(block)+" 4 "+String.valueOf(readSize));
    System.arraycopy(fileReader.getRawImageData(),(int)(dataSize * block),buffer,4,readSize);
   // Log.i("filefinal", "buffer "+Arrays.toString(buffer));
   // Log.i("filebuffer", Arrays.toString(fileReader.getRawImageData()));


     // Log.i("filebuffer"+String.valueOf(block), String.valueOf(buffer[0])+"=="+String.valueOf(buffer[buffer.length-1]));
  ...
  public void sendEoadEnableImageCmd() {
    ...
    Log.i("mkl", "sendEoadEnableImageCmd: with "+imageControlChar.getUuid().toString()+" value "+ cmd);
    oadDevice.writeCharacteristicAsync(imageControlChar,cmd);
  }
  ...
  public void sendEoadStartOADProcessCmd () {
    ...
    Log.i("mkl", "sendEoadStartOADProcessCmd: "+cmd);
    oadDevice.writeCharacteristicAsync(imageControlChar,cmd);
  }
  ...
  public void handleOADControlPointMessage(final byte[] response) {
    Log.i("res", Arrays.toString(response));

    switch(response[0]) {
      case TIOADEoadDefinitions.TI_OAD_CONTROL_POINT_CMD_DEVICE_TYPE_CMD:
        Log.i("c1", Arrays.toString(response));
      ...
      case TIOADEoadDefinitions.TI_OAD_CONTROL_POINT_CMD_GET_BLOCK_SIZE:
        Log.i("c2", String.valueOf(TIOADEoadDefinitions.BUILD_UINT16(response[2],response[1])));
        Log.i("mklda", "got block sixe "+String.valueOf(TIOADEoadDefinitions.BUILD_UINT16(response[2],response[1])));
      ...
      case TIOADEoadDefinitions.TI_OAD_CONTROL_POINT_CMD_ENABLE_OAD_IMAGE:
        Log.i("mklda", "TI_OAD_CONTROL_POINT_CMD_ENABLE_OAD_IMAGE");

        Log.i("c3", Arrays.toString(response));
        if (response[1] == 0x00) {
          Log.i("mklda", "TI_OAD_CONTROL_POINT_CMD_ENABLE_OAD_IMAGE OK");
      ...
      case TIOADEoadDefinitions.TI_OAD_CONTROL_POINT_CMD_IMAGE_BLOCK_WRITE_CHAR_RESPONSE:
        Log.i("c4", Arrays.toString(response));
        Log.i("mklda", "TI_OAD_CONTROL_POINT_CMD_IMAGE_BLOCK_WRITE_CHAR_RESPONSE "+Arrays.toString(response));
        if (response[1] == 0x00) {
          if (internalState == TIOADEoadDefinitions.oadStatusEnumeration.tiOADClientProgrammingAbortedByUser) {
          ...
          new Thread(new Runnable() {
            ...
            public void run() {
              ...
              Log.i("mklda", "going to send block run: "+block);
              Log.i("mklda", "going to send block run: "+Arrays.toString(response));
      ...
      case TIOADEoadDefinitions.TI_OAD_CONTROL_POINT_CMD_START_OAD_PROCESS:
        Log.i("c5", Arrays.toString(response));
        Log.i("mklda", "going to send TI_OAD_CONTROL_POINT_CMD_START_OAD_PROCESS run: "+Arrays.toString(response));
        break;
      default:
  ...
  BluetoothLEDevice.BluetoothLEDeviceCB cb = new BluetoothLEDevice.BluetoothLEDeviceCB() {
    ...
    public void didUpdateCharacteristicData(BluetoothLEDevice dev, final BluetoothGattCharacteristic characteristic) {
      Log.i("res2", Arrays.toString(characteristic.getValue()));

      Log.d("mklda","Characteristic: " + characteristic.getUuid().toString() + " Value: " +Arrays.toString(characteristic.getValue()));
      if (characteristic.getUuid().toString().equalsIgnoreCase(TIOADEoadDefinitions.TI_OAD_IMAGE_CONTROL)) {
         Log.d("mklda","TI_OAD_IMAGE_CONTROL " + characteristic.getUuid().toString() + " Value: " +Arrays.toString( characteristic.getValue()));

        handleOADControlPointMessage(characteristic.getValue());
      }
      else if (characteristic.getUuid().toString().equalsIgnoreCase(TIOADEoadDefinitions.TI_OAD_IMAGE_NOTIFY)) {
        Log.d("mklda","TI_OAD_IMAGE_NOTIFY " + characteristic.getUuid().toString() + " Value: " +Arrays.toString( characteristic.getValue()));
    ...
    public void didWriteCharacteristicData(BluetoothLEDevice dev, BluetoothGattCharacteristic characteristic) {
      Log.d(TAG,"didWriteCharacteristic: " + characteristic.getUuid().toString() + " Value: " + TIOADEoadDefinitions.BytetohexString(characteristic.getValue())+" / "+Arrays.toString(characteristic.getValue()));
    }
